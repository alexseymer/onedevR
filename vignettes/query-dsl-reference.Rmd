---
title: "OneDev Query DSL Reference"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{OneDev Query DSL Reference}
...
The **OneDev Query DSL** (Domain-Specific Language) is a powerful filtering syntax used across issues, pull requests, builds, and projects. This vignette provides a comprehensive reference for constructing queries programmatically, with special attention to patterns useful for AI and LLM-based automation.
...
- **Special values** (functions, operators) are unquoted: `currentUser()`, `true`, `false`
...
This section addresses the unique challenges of **programmatic query construction** using LLMs and automated systems.

## Constructing Queries Programmatically

When building queries dynamically (e.g., in an LLM agent or automation script), follow these patterns:
...
When accepting user input (especially from LLMs), escape special characters:
...
- **gitlabr (design reference)**: https://thinkr-open.github.io/gitlabr/
- **onedevr Issues**: https://github.com/alexseymer/onedevR/issues

For more on using queries with onedevr functions:

- `vignette("getting-started", package = "onedevr")` — Basic setup
- `vignette("issues-workflow", package = "onedevr")` — Issue patterns
- `vignette("builds-workflow", package = "onedevr")` — Build patterns
- `vignette("pull-requests-workflow", package = "onedevr")` — PR patterns
