...
vignette: >
  %\VignetteIndexEntry{Pull Requests Workflow}
...
```r
# Request changes (if function available)
od_request_review_changes(
  5,
  message = "Please update the error handling per the comments"
)
```
...
```r
# Merge a PR (if function available)
od_merge_pull_request(
  5,
  commit_message = "Add OAuth2 authentication (#5)",
  method = "squash"  # or "merge" or "rebase"
)

# Close without merging (if needed)
od_close_pull_request(5, reason = "Duplicate of #3")
```
