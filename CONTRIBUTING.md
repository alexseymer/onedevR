...
Thank you for your interest in contributing to onedevR! This guide will help you get started.
...
1. **Create function in appropriate R file** (or new file if it's a new module)
...
- **Function names:** `od_action_noun()` (e.g., `od_get_issue()`, `od_create_issue()`)
...
Key references:
- `tod` CLI: https://github.com/theonedev/tod
- OneDeV API: `https://your-onedev-host/~help/api`
...
- Use mocks for API calls (no live API calls in tests)
...
- `@examples` with `\dontrun{}` for interactive examples
